...
Open http://localhost:3000. You can alternatively set `GEMINI_API_KEY` to use Gemini or `OPENAI_API_KEY` to use OpenAI. Provider priority is Groq, then Gemini, then OpenAI. The Groq model is `openai/gpt-oss-20b`. Remove any old `GROQ_MODEL` environment variable in Render; it is no longer used. The default Gemini model is `gemini-3.1-flash-lite`; override with `GEMINI_MODEL` if necessary. Set `PORT` for a different port.
